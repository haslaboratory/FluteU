...
class DualPortBRAM(DATA_WIDTH: Int = 32, DEPTH: Int = 1024, LATENCY: Int = 1) extends BlackBox(Map(
...
class DualPortByteWriteBRAM(DATA_WIDTH: Int = 32, DEPTH: Int = 1024, LATENCY: Int = 1) extends BlackBox(Map(
  "DATA_WIDTH" -> DATA_WIDTH,
  "DEPTH" -> DEPTH,
  "LATENCY" -> LATENCY)) with HasBlackBoxInline {

  override val desiredName = "dual_port_byte_write_bram"
  ...
  setInline("dual_port_byte_write_bram.v",
    s"""
       |module dual_port_byte_write_bram #(
       |	parameter DATA_WIDTH = 32,
       |	parameter DEPTH      = 1024,
       |	parameter LATENCY    = 1,
       |	parameter LATENCY_A  = LATENCY,
